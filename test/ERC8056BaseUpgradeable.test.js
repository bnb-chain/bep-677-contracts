...
describe("ERC8056BaseUpgradeable (via ScaledUITokenUpgradeable)", function () {
  let proxy, owner, other;

  beforeEach(async function () {
    [owner, other] = await ethers.getSigners();
    const Factory = await ethers.getContractFactory("ScaledUITokenUpgradeable");
  ...
  describe("conversion math", function () {
    const ONE_TOKEN = 10n ** 18n; // 1 raw token (18 decimals)
    ...
    it("balanceOfUI / totalSupplyUI scale with multiplier", async function () {
      ...
      expect(await proxy.totalSupplyUI()).to.equal(rawTotal); // 1x → same
      ...
      // raw supply unchanged; UI supply doubled
      expect(await proxy.totalSupply()).to.equal(rawTotal);
    ...
    it("UIMultiplierUpdated event emits correct oldMultiplier after lapsed seal", async function () {
      ...
      // Schedule B — seals A; UIMultiplierUpdated.oldMultiplier = multA (currently active)
      const newBlock = await ethers.provider.getBlock("latest");
      const multB = 3n * MULTIPLIER_DECIMALS;
  ...
  describe("upgrade safety", function () {
    it("validates safe V2 upgrade (appended field)", async function () {
      const V2Factory = await ethers.getContractFactory("ScaledUITokenUpgradeableV2Mock");
      await upgrades.validateUpgrade(await proxy.getAddress(), V2Factory, { kind: "uups" });
    });
  });
});
